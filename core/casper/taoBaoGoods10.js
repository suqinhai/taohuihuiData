...
casper.then(function() {
    ...
    this.waitForSelector('.qrcode-img img', function() {
        ...
        var loginCode = this.evaluate(function() {
            ...
            __utils__.sendAJAX('http://172.16.1.56:3001/taohuihui/public/sendEmail', 'post', {
                'text': '淘宝联盟登录二维码',
                'url': 'http:' + code,
                'title': '淘宝联盟登录二维码',
    ...
    this.waitForUrl(/index\.htm$/, function() {
        ...
        casper.open('http://pub.alimama.com/promo/search/index.htm');

        casper.then(function() {
            var datas = [];
            var category = this.evaluate(function() {
                var data = __utils__.sendAJAX('http://172.16.1.56:3001/taohuihui/classify/getThirdProperty', 'get', false);
                var category = [];
...
function pageCb(category, toPage, perPageSize, callback) {
    ...
    if (items) {
        ...
        var auctionIds = casper.evaluate(function(items) {
            ...
            var auctionIds = __utils__.sendAJAX('http://172.16.1.56:3001/taohuihui/goods/verifyId', 'post', { data: JSON.stringify(auctionIds) }, false);
        ...
        async.mapLimit(dataArr, 1, function(item, callback2) {
            ...
            casper.evaluate(function(data) {
                __utils__.sendAJAX('http://172.16.1.56:3001/taohuihui/goods/add', 'post', {
